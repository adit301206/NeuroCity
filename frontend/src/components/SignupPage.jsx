  ...
  async function handleVerifyOtp(e) {
    ...
    try {
      ...
      if (res.status === 200 && data.token) {
        ...
        setError(data.message || 'Incorrect or expired OTP verification code.');
      }
    } catch (err) {
        ...
        <div className="right">
          <div className="form-wrap">
            {!otpStep ? (
              ...
              <>
                <div className="form-head">
                  <div className="tag" style={{ background: 'rgba(16, 185, 129, 0.15)', color: '#10B981' }}>OTP activation</div>
                  <h2>Verify Email Address</h2>
                  <p>A verification code has been dispatched to <strong>{email}</strong>. Enter it below to complete citizenship setup.</p>
                </div>
                ...
                <form onSubmit={handleVerifyOtp} noValidate>
                  ...
                  <button type="submit" className="submit" disabled={submitting}>
                    {submitting ? 'Verifying Code…' : 'Activate Account & Sign in'}
                  </button>
